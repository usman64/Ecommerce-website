...
export const addCollectionAndDocuments = async (collectionKey, objectToAdd) => {
  ...
  console.log(collectionRef); //No matter what even if Ref obj empty it firebase will return it
  //we can only make 1 set call at a time
  //Problem: Set calls run 1 at time and if internet connection drops in b/w so unrelaiblilty arises
  //Solution: firebase Batch write
  const batch = firestore.batch();
  objectToAdd.forEach((obj) => {
    const newDocRef = collectionRef.doc(); //unique key firebase sets itself
